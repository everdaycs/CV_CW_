...
def build_targets(p, targets, model):
    ...
    for i in range(det.nl):
        ...
        if nt:
            ...
            t = t[j]  # filter
        # If no targets for this layer, append empty placeholders
        if t.numel() == 0:
            empty_int = torch.tensor([], device=targets.device, dtype=torch.long)
            empty_float4 = torch.zeros((0, 4), device=targets.device)
            indices.append((empty_int, empty_int, empty_int, empty_int))
            tbox.append(empty_float4)
            anch.append(anchors[empty_int])
            tcls.append(empty_int)
            continue

        # Offsets and repeat logic
        gxy = t[:, 2:4]  # grid xy
        ...
        # Define (avoid .T on higher-dim tensors)
        tc = t[:, :2].long()
        b = tc[:, 0]  # batch indices
        c = tc[:, 1]  # class labels
        gxy = t[:, 2:4]
        gwh = t[:, 4:6]
        ...
        tbox.append(torch.cat((gxy - gij, gwh), 1))  # boxes
        anch.append(anchors[a])  # anchors
        tcls.append(c)  # classes
...
def output_to_target(output, width, height):
    ...
    for i, o in enumerate(output):
        if o is None:
            continue
        # Move torch Tensor to CPU and convert to numpy if needed
        if isinstance(o, torch.Tensor):
            o = o.cpu().numpy()
        for pred in o:
            # pred now a numpy array
            box = pred[:4]
            w = (box[2] - box[0]) / width
